    ...
    <div className="card-premium p-6 sm:p-8">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2">
                <span
                  className={`flex items-center justify-center w-7 h-7 rounded-full font-mono text-[13px] tracking-wider transition-colors ${
                    isActive
      ...
      >
        <div className="lg:col-span-5">
          <p className="font-mono text-[13px] tracking-[0.16em] text-primary">STEP {current.n}</p>
        ...
        <div className="lg:col-span-7 space-y-5">
          ...
          <div className="rounded-xl border border-border bg-card/40 p-5">
            <p className="font-mono text-[13px] uppercase tracking-widest text-body mb-2">Output</p>
            <p className="text-sm text-body">{current.out}</p>
          </div>
          <div className="flex items-center gap-3 pt-2">
            <button
              onClick={() => setActive((active - 1 + steps.length) % steps.length)}
              className="font-mono text-[13px] tracking-[0.2em] text-body hover:text-foreground transition-colors"
              aria-label="Previous step"
            >
              ← PREV
            </button>
            <span className="font-mono text-[13px] text-body" aria-hidden>
              {active + 1} / {steps.length}
            </span>
            <button
              onClick={() => setActive((active + 1) % steps.length)}
              className="font-mono text-[13px] tracking-[0.2em] text-primary hover:text-primary/80 transition-colors"
